...
Parse.Cloud.define('quarterly-report-retrieve', async ({ params: { quarter }, master }) => {
...
async function processQuarter (quarter) {
  ...
  for (const row of rows) {
    ...
    if (regionId) {
      if (!regionals[regionId]) {
        regionals[regionId] = {
          regionId,
          name: await getOrCacheRegionalCommissions().then(regions => regions.find(region => region.regionId === regionId).name),
          total: 0,
          cubes: 0,
          orders: {}
        }
